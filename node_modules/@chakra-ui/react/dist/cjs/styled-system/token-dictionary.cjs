...
var resolveTokenValue = require('./resolve-token-value.cjs');
...
function expandBreakpoints(breakpoints) {
  ...
  return {
    ...
    sizes: entries.mapEntries(breakpoints, (key, value) => [
      `breakpoint-${key}`,
      { value }
    ])
  };
  ...
  function registerTokens() {
    walkObject.walkObject(
      ...
      (entry, path) => {
        ...
        const resolved = resolveTokenValue.resolveTokenValue(category, t.value);
        const token = {
          value: resolved,
    ...
    walkObject.walkObject(
      ...
      (entry, path) => {
        ...
        const category = path[0];
        const name = formatTokenName(path);
        const t = is.isString(entry.value) || Array.isArray(entry.value) ? { value: { base: entry.value } } : entry;
        const cond = resolveTokenValue.resolveSemanticConditionValues(category, t.value);
        const token = {
          value: cond.base ?? "",
...
function getConditionalTokens(token) {
  ...
  walkObject.walkObject(conditions, (value, path) => {
    ...
    const nextToken = {
      ...
      value,
      extensions: {
        ...omit.omit(token.extensions, ["conditions"]),
